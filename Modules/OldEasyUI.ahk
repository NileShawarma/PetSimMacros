...
CreateBaseUI(MapIndex) {
    ...
    BaseGui.AddText("w240 h20 x20 yp+25", "Version: " MapIndex["Main"].Version).SetFont("s13 w500")
    BaseGui.AddLink("w240 h60 x20 yp+20", 'Macro Made By Nile Shawarma / I completely stole <a href="https://www.youtube.com/channel/UCKOkQGvHO71nqQjwTiJX5Ww">A Basements</a> code for the UI, so give him a sub').SetFont("s11 w500")
    BaseGui.AddText("w200 h" MapIndex["Main"].DescY " x20 yp+65 Wrap", MapIndex["Main"].Description).SetFont("s10 w500")

    EMB := BaseGui.AddButton("w100 h50 xs y350", "Enable Macro")
    YTB := BaseGui.AddButton("w140 h50 xp+100 y350", "A Basement's YT Channel")
            ...
            switch SettingObject.Type {
                case "Position":
                    for Key, Value in SettingObject.Map {
                        if (Value.Length==2){
                            If SettingObject.SaveName!="PositionSettings"{
                                MsgBox Value.Length
                            }
                            IniWrite(Value[1] "|" Value[2], FileToSaveTo, SettingObject.SaveName, Key)
                        }
                        else if(Value.Length==4){
                            IniWrite(Value[1][1] "|" Value[1][2] "|" Value[2][1] "|" Value[2][2] "|" Value[3][1] "|" Value[3][2] "|" Value[4][1] "|" Value[4][2], FileToSaveTo, SettingObject.SaveName, Key)
                            MsgBox "Wrote"
                            MsgBox Value[1][1] "|" Value[1][2] "|" Value[2][1] "|" Value[2][2] "|" Value[3][1] "|" Value[3][2] "|" Value[4][1] "|" Value[4][2]
                        }
                        else{
                            MsgBox Value.Length
                        }
                ...
                case "Colour":
                    for Key, Value in SettingObject.Map {
                        if (Value.Length==2){
                            ...
                            IniWrite(Value[1] "|" Value[2] "|" Value[3] "|" Value[4], FileToSaveTo, SettingObject.SaveName, Key)
                        }
                    }
                case "Selection":
                    FormattedText := ""
                        ...
                        try {
                            switch SettingObject.type {
                                case "Position":
                                    ...
                                    if (SettingObject.Map[Key].Length==2){
                                    ...
                                    else if(SettingObject.Map[Key]==8){
                                        VariablisticMap[SettingObject.SaveName][Key][1].Value := SettingObject.Map[Key][1]
                                        VariablisticMap[SettingObject.SaveName][Key][2].Value := SettingObject.Map[Key][2]
                                        VariablisticMap[SettingObject.SaveName][Key][3].Value := SettingObject.Map[Key][3]
                                        VariablisticMap[SettingObject.SaveName][Key][4].Value := SettingObject.Map[Key][4]
                                    }
                                case "Colour":
                                    SettingObject.Map[Key] := StrSplit(IniRead(LoadFile, SettingObject.SaveName, Key), "|")
                                    MsgBox SettingObject.Map[Key][1]
                                    if(SettingObject.Map[Key]==4){
                                        VariablisticMap[SettingObject.SaveName][Key][1].Value := SettingObject.Map[Key][1]
                                        VariablisticMap[SettingObject.SaveName][Key][2].Value := SettingObject.Map[Key][2]
                                        VariablisticMap[SettingObject.SaveName][Key][3].Value := SettingObject.Map[Key][3]
                                        VariablisticMap[SettingObject.SaveName][Key][4].Value := SettingObject.Map[Key][4]
...
CreatePosHelper(UI, Name, PosArray, Num, I := "", Objective := false) {
    ...
    if (PosArray.Length==2){
        if Objective { ;Still dont know what Objective changes, Responsible for creating the editable X and Y value UI's
        ...
        } else { ;Responsible for creating the editable X and Y value UI's
    ...
    else if (PosArray.Length==4){ ;I made this part
        if Objective { ;Still dont know what Objective changes, Responsible for creating the editable X and Y value UI's
            Ud1 := UI.Add("Edit","ys w60 x" (250-PushButton),)
            UI.AddUpDown("v" Name I "XPos Range1-40000", PosArray[1])
            ud2 := UI.Add("Edit","ys w60 x" (320-PushButton),)
            ...
            Ud1 := UI.Add("Edit","ys w60 x250",)
            UI.AddUpDown("v" Name "XPos Range1-40000", PosArray[1][1]) 
            ud2 := UI.Add("Edit","ys w60 x320",)
            UI.AddUpDown("v" Name "YPos Range1-40000", PosArray[1][2])
            ud3 := UI.Add("Edit","ys w60 x390",)
            UI.AddUpDown("v" Name "X2Pos Range1-40000", PosArray[2][1]) 
            ud4 := UI.Add("Edit","ys w60 x450",)
            UI.AddUpDown("v" Name "Y2Pos Range1-40000", PosArray[2][2])
            Ud5 := UI.Add("Edit","ys w60 x520",)
            UI.AddUpDown("v" Name "X3Pos Range1-40000", PosArray[3][1]) 
            ud6 := UI.Add("Edit","ys w60 x590",)
            UI.AddUpDown("v" Name "Y3Pos Range1-40000", PosArray[3][2])
            ud7 := UI.Add("Edit","ys w60 x660",)
            UI.AddUpDown("v" Name "X4Pos Range1-40000", PosArray[4][1]) 
            ud8 := UI.Add("Edit","ys w60 x730",)
            UI.AddUpDown("v" Name "Y4Pos Range1-40000", PosArray[4][2])
        }
    }
    ButtonClicked(*) {
...
CreateColHelper(UI, Name, PosArray, Num, I := "", Objective := false) {
    PushButton := 0
    if Objective {
        PushButton += 48
        UI.Add("Text","Section xs y" Num, Name ":")
    ...
    UI.Add("Button", "w25 h20 x" (220 - PushButton) " ys", "S").OnEvent("Click", ButtonClicked)
    Ud1 := ""
    ud2 := ""
    if (PosArray.Length==2){
        if Objective { ;Still dont know what Objective changes, Responsible for creating the editable X and Y value UI's
        ...
        if Objective { ;Still dont know what Objective changes, Responsible for creating the editable X and Y value UI's
            ...
            UI.AddEdit("w60 h20 yp xp+35 v" Name "1", PosArray[1])
            UI.AddEdit("w60 h20 yp xp+65 v" Name "2", PosArray[2])
            UI.AddEdit("w60 h20 yp xp+65 v" Name "3", PosArray[3])
            UI.AddEdit("w60 h20 yp xp+65 v" Name "4", PosArray[4])
        }
    }
    ButtonClicked(*) {
        global CurrentPostionLabel := [UD1, UD2]
...
Create_TNTP_UI(_MapOBJ, BaseUI) {
    ...
    loop (Ceil(_Map.Count / 15)) {
        TabsArray.InsertAt(TabsArray.Length + 1, "Settings[" A_Index "]") ;Creates tab names i think
    }

    TSTabs := SettingsUI.AddTab3("", TabsArray)
    if not _MapOBJ.Type = "Position" { ;Creates the first page of each setting UI??
        ...
        for Setting, SettingValue in _Map {
            ...
            else if(SettingValue.Length==4){
                SettingsUI.SetFont("s15 q5 w800", "Constantia")
                SettingsUI.Add("Text", "Section", "Positioning")
                SettingsUI.Add("Text"," ys+5 x270 c0x000000","X")
                SettingsUI.Add("Text"," ys+5 x340 c0x000000","Y")
                SettingsUI.Add("Text"," ys+5 x410 c0x000000","X2")
                SettingsUI.Add("Text"," ys+5 x480 c0x000000","Y2")
                SettingsUI.Add("Text"," ys+5 x550 c0x000000","X3")
                SettingsUI.Add("Text"," ys+5 x620 c0x000000","Y3")
                SettingsUI.Add("Text"," ys+5 x690 c0x000000","X4")
                SettingsUI.Add("Text"," ys+5 x760 c0x000000","Y4")
                SettingsUI.SetFont("s9 q5 w500", "Arial")
                break
            }
        }
    ...
    for Setting, SettingValue in _Map {
        ...
        if NumericalSetting >= 16 { ;If theres 16 elements per page, a new page will be created for the next bunch of settings
            NumericalSetting := 1

            ;- WE LOVE COPY AND PASTING CODE
            if not _MapOBJ.Type = "Position" { ;Creates as many pages of UI as required
                SettingsUI.AddText("w200 h20 Section", _MapOBJ.Type " Settings").SetFont("s12 w700")
            } else {
                SettingsUI.SetFont("s15 q5 w800", "Constantia")
                SettingsUI.Add("Text", "Section", "Positioning")
                SettingsUI.Add("Text"," ys+5 x270 c0x000000","X")
                SettingsUI.Add("Text"," ys+5 x340 c0x000000","Y") ;Creates the headers of X, Y and Positioning
                SettingsUI.SetFont("s9 q5 w500", "Arial")
            }
        }
        ...
        switch _MapOBJ.Type {
            case "Toggle":
                ...
                VariablisticMap[_MapOBJ.SaveName][Setting] := SettingsUI.AddCheckbox("w20 h20 v" Setting " yp xp+240 Checked" SettingValue) ;Basically if its a toggle settin its gonna create a toggle button + Text and set the value to whatever is saved atm
            ...
            case "Number":
                ...
                VariablisticMap[_MapOBJ.SaveName][Setting] := SettingsUI.AddEdit("w120 h20 yp xp+190") ;idk wtf addedit() does (think it just adds a textbox for some reason)
                SettingsUI.AddUpDown("v" Setting " range1-10000000" Setting, SettingValue) ;Creates a lil number entering UI + text to tell the user what it does which u can scroll with
            case "Text":
                SettingsUI.AddText("w150 h20 xs y" (NumericalSetting * 25 + (40)), Setting ":").SetFont("s10")
                VariablisticMap[_MapOBJ.SaveName][Setting] := SettingsUI.AddEdit("w120 h20 yp xp+190 v" Setting, SettingValue) ;I still dont know wtf addedit() does but im guessing in this case it just adds a textbox
            case "Position":
                VariablisticMap[_MapOBJ.SaveName][Setting] := CreatePosHelper(SettingsUI, Setting, SettingValue, NumericalSetting) ;ffs have to read a different function
            case "Colour":
                VariablisticMap[_MapOBJ.SaveName][Setting] := CreateColHelper(SettingsUI, Setting, SettingValue, NumericalSetting)
        }
    }
        ...
        switch _MapOBJ.Type {
            case "Position":
                for Key, Value in _Map {
                    ...
                    if (ReturnedValues.Count==16){
                        _Map[Key] := [[ReturnedValues[Key "XPos"], ReturnedValues[Key "YPos"]],[ReturnedValues[Key "X2Pos"], ReturnedValues[Key "Y2Pos"]],[ReturnedValues[Key "X3Pos"], ReturnedValues[Key "Y3Pos"]],[ReturnedValues[Key "X4Pos"], ReturnedValues[Key "Y4Pos"]]]
                    }
                }
            case "Colour":
                for Key, Value in _Map {
                    _Map[Key] := [ReturnedValues[Key "1"], ReturnedValues[Key "2"], ReturnedValues[Key "3"], ReturnedValues[Key "4"]]
                    if (ReturnedValues.Count==16){
                        _Map[Key] := [ReturnedValues[Key "XPos"], ReturnedValues[Key "YPos"],ReturnedValues[Key "X2Pos"], ReturnedValues[Key "Y2Pos"],ReturnedValues[Key "X3Pos"], ReturnedValues[Key "Y3Pos"],ReturnedValues[Key "X4Pos"], ReturnedValues[Key "Y4Pos"]]
                    }
                }
